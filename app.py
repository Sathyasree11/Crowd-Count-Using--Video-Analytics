...
DB_CONFIG = {
    ...
    'database': os.environ.get('MYSQL_DB', 'flask_zone_app1'),
...
# Function to get zones counts over time
def get_zones_data():
    # Example: read from CSV or database
    try:
        with open('data/zones.csv', 'r') as f:
            ...
            for row in reader:
                ...
                zone_counts.append(int(row['count']))
        return {"timestamps": timestamps, "counts": zone_counts}
    except FileNotFoundError:
        return {"timestamps": [], "counts": []}

# Function to get activity counts over time
def get_activity_data():
    # Example: read from CSV or database
    try:
        with open('data/activity.csv', 'r') as f:
            reader = csv.DictReader(f)
            timestamps = []
            activity_counts = []
            for row in reader:
                timestamps.append(row['timestamp'])
                activity_counts.append(int(row['people']))
        return {"timestamps": timestamps, "counts": activity_counts}
    except FileNotFoundError:
        return {"timestamps": [], "counts": []}

# Function to get heatmap points
def get_heatmap_points():
    # Example: read from JSON or database
    try:
        with open('data/heatmap.json', 'r') as f:
            points = json.load(f)
        return points  # Example: [{"x":100,"y":50,"value":2}, ...]
    except FileNotFoundError:
        return []
    ...
    try:
        with conn.cursor() as cur:
            if request.method == 'POST':
                ...
                fields = []
                params = []
                for field_name, value in [('full_name', full_name), ('email', email), ('phone', phone),
                                          ('dob', dob), ('gender', gender), ('address', address), ('about', about)]:
                    if value:
...
@app.route('/graphs')
def graphs_page():
    return render_template('graphs.html', file=request.args.get('file',''), user=session.get('username'))

@app.route('/alerts')
def alerts_page():
    return render_template('alerts.html', file=request.args.get('file',''), user=session.get('username'))

@app.route('/api/recent_counts')
def api_recent_counts():
    filename = request.args.get('file') or ''
    limit = int(request.args.get('limit','300'))
    user_id = session.get('user_id')
    data = { 'labels': [], 'series': {}, 'current': {} }
    conn = get_db_connection()
    if not conn:
        return jsonify(data)
    with conn:
        with conn.cursor() as cur:
            vid = None
            if filename:
                cur.execute("SELECT id FROM videos WHERE filename=%s AND (user_id=%s OR %s IS NULL)", (filename, user_id, user_id))
                row = cur.fetchone()
                if row: vid = row['id']
            if not vid and user_id:
                cur.execute("SELECT id FROM videos WHERE user_id=%s ORDER BY id DESC LIMIT 1", (user_id,))
                row = cur.fetchone()
                if row: vid = row['id']
            if not vid:
                return jsonify(data)
            cur.execute(
                "SELECT zone_id, ts, label, current FROM zone_counts WHERE video_id=%s ORDER BY id DESC LIMIT %s",
                (vid, limit)
            )
            rows = cur.fetchall() or []
            rows.reverse()
            # Build labels (timestamps) and series per zone
            labels = []
            zone_to_points = {}
            for r in rows:
                t = r['ts']
                labels.append(t)
                zid = r['zone_id']
                zone_to_points.setdefault(zid, {'label': r['label'] or 'Zone', 'points': []})
                zone_to_points[zid]['points'].append(int(r['current'] or 0))
            # Deduplicate labels
            seen = set()
            uniq_labels = []
            for t in labels:
                if t not in seen:
                    seen.add(t)
                    uniq_labels.append(t)
            data['labels'] = uniq_labels
            # Current by the latest point for each zone
            for zid, obj in zone_to_points.items():
                data['series'][zid] = { 'label': obj['label'], 'data': obj['points'] }
                if obj['points']:
                    data['current'][zid] = obj['points'][-1]
            return jsonify(data)
        ...
        with conn:
            with conn.cursor() as cur:
                try:
                    ...
                    cur.execute(
                        "INSERT INTO users (username, email, full_name, phone, password_hash) VALUES (%s, %s, %s, %s, %s)",
